...
/**
 * @desc Apply for a job (Applicant)
...
export const applyForJob = async (req, res) => {
  try {
    ...
    // FIXED: Proper signed upload with PDF format for clean URLs
    ...
    const resumeUrl = uploadResult.secure_url; // ✅ Clean single URL: https://res.cloudinary.com/.../resumes/abc.pdf
    const cloudinaryPublicId = uploadResult.public_id;

    const newApplication = {
      jobId: new ObjectId(jobId),
      applicantId: new ObjectId(applicantId),
      resumeUrl, // ✅ Single clean URL for view/download
      cloudinaryPublicId,
    ...
    return res.status(201).json({
      ...
      data: {
        ...
        resumeUrl: newApplication.resumeUrl, // ✅ Use this URL directly
...
export const withdrawApplication = async (req, res) => {
  try {
    ...
    // Only delete from DB (no Cloudinary destroy to avoid signature issue)
...
/**
 * @desc Get applications by job (HR/Admin)
...
/**
 * @desc Download/Get resume URL - FIXED ✅
...
export const downloadResume = async (req, res) => {
  try {
    ...
    // ✅ Direct redirect to Cloudinary PDF (view/download works perfectly)
...
export const getApplicationStats = async (req, res) => {
  try {
    ...
    const [total, pending, shortlisted, selected, rejected] = await Promise.all(
      [
